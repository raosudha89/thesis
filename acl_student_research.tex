...
\begin{enumerate}[noitemsep,nolistsep]
	...
	\item Does the EVPI formalism provide leverage over a similarly expressive feed-forward network? %In particular, is EVPI a useful inductive bias for a model?
...
For every post $p$ in our dataset of $(p, q, a)$ triples, we label the question $q$ paired with $p$ as positive. We define our two setups based on how we get our negative question candidates for every $p$. For the \textbf{lucene negative candidates} setup, we retrieve nine question candidates using Lucene (\S\ref{question_candidate_generator}). For the \textbf{random negative candidates} setup, we randomly sample nine other questions from our dataset.

\subsection{Baseline methods}\label{baselines}

 Given a post, we randomly permute its set of 10 candidate questions uniformly to get our random baseline. For our bag-of-ngrams baseline,  we construct a bag-of-ngrams representation for the post, the question and the answer and train a classifier to minimize hinge loss on misclassification loss. For our feedforward neural baseline, we concatenate the post LSTM representation, the question LSTM representation and the answer LSTM representation and feed it through a feed forward neural network of two fully-connected hidden layers.

\subsection{Results}

We describe results on a test split of askubuntu when our models are trained on the union of all data, summarized in Table~\ref{results_topN}. The left half of this table shows results when the candidate sets is from Lucene---the ``hard'' setting.
Here, we see that for all the evaluation metrics:
EVPI outperforms the neural feedforward model that does not incorporate the EVPI formalism;
...
A final performance of 51\% recall at 3 is encouraging, though clearly there is a long way to go for a perfect system. The right half of this table shows the same results when the candidate set is chosen randomly---the ``easy'' setting.

\section{Proposed research directions}

In our preliminary work, we focused on building a model that can learn to select a question to ask from a candidate set of prior questions. To make the system more general, it needs to be able to generalize to new context well. Towards this goal, we propose two new research directions:

\subsection{Template based question generation}

 Consider a template like ``What version of \_\_\_  are you running?''. This template can generate thousands of specific variants found in the data like ``What version of Ubuntu are you running?'',  ``What version of apt-get are you running?'', etc. We propose a following four step approach to our template based clarification question generation method:
\begin{enumerate}[noitemsep,nolistsep]
	\item Identifying groups of similar questions from a huge collection of questions.
	\item Generating a template for each group of questions.
	\item Given a post, selecting the right question template from a set of candidate templates.
	\item Filling in the blanks in the template using relevant context from the given post.
\end{enumerate}

For grouping similar questions together, we will plan to use some of the clustering algorithms which cluster documents based on their lexical and semantic similarity. To generate templates corresponding to each cluster, we plan to identify topic specific words from the question using syntactic information (like parts-of-speech tags) and then remove them to form generic templates.
In the third step, given a post and a set of candidate question templates, we will select a template using a model similar to our preliminary work. Finally, in the fourth step, we explore methods similar to slot-filling approaches \cite{} to populate the blanks in the template with post specific words.

\subsection{Neural network generative model}

Sequence to sequence models \cite{} have proven to be effective for several language generation tasks like machine translation \cite{}, dialog generation \cite{}, etc. In our second research direction, we plan to build a similar encoder-decoder framework for generating a clarification question one word at a time, given the words of a post. A recent neural generative question answering model \cite{} %https://arxiv.org/pdf/1512.01337.pdf
  built an answer language model which decides, at each time step, whether to generate a common vocabulary word or a answer word retrieved from a knowledge base. Inspired from this work, we plan to explore a similar question generation model which will decide, at each time step, whether to generate a common vocabulary word or a topic specific word retrieved from the given post. This idea will thus try to incorporate the template generation method into a more general neural network framework.

\section{Conclusion}

In our preliminary work we described a new dataset for question generation, and attacked this problem from the perspective of question selection. This model integrates state-of-the-art neural network structure with the classic notion of expected value of perfect information, which effectively models a pragmatic choice on the part of the questioner: how do I \emph{imagine} the other party would answer if I were to ask this question. Such pragmatic priciples have recently been shown to be useful in other tasks as well \cite{golland2010game,smith2013learning,orita2015discourse,andreas2016reasoning}.
