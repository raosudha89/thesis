...
For every post $p$ in our dataset of $(p, q, a)$ triples, we label the question $q$ paired with $p$ as positive. We define our two setups based on how we get our negative question candidates for every $p$:\\
\textbf{Lucene negative candidates:} We retrieve nine question candidates using Lucene (\S\ref{question_candidate_generator}).\\
...
\subsection{Template based question generation}
 ...
 Consider a template like ``What version of \_\_\_  are you running?''. This template can generate thousands of specific variants found in the data like ``What version of Ubuntu are you running?'', ``What version of OS are you running?'', ``What version of apt-get are you running?'', etc. We identify four components to our template based question generation method:
\begin{enumerate}[noitemsep,nolistsep]
	\item Identifying near similar questions from a huge collection of questions
	\item Generating templates from near similar questions
...
For identifying similar questions, we plan to use clustering algorithms which cluster documents based on their similarity. Given a set of questions that are near similar, the next step is to extract a generic template which can be expanded to (almost) each of the variants in the cluster. We use the parts-of-speech information of the words in the question to identify proper nouns (and others) that indicate specific topics. Given these markings, we remove them from the questions to generate a template. Further, we classify questions into probable question types using an idea similar to \cite{}. Given a post, the next step then is to generate a set of candidate question templates and then to select the most relevant question template from the set of candidates. We use our previous model for this step.
