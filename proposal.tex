...
%this para -- we also explore question generation in dialogue setting. work in dialogue -- next utterance classification -- moving on to generating questions in multi-turn dialogue -- generative models on ubuntu dialogue dataset

%
%Asking questions is fundamental to communication. Questions serve different purposes in language. For instance, information retrieval from a knowledge base (``What is the capital of United States''), 

%- clarification question: When passing some information from person A to person B through language, person A has certain understanding of the subject and he/she communicates it across with that context in mind. This context can be very different from person B's understanding of the same subject. Given the possibility of differences between the two understanding, two people are 
%!TEX encoding = UTF-8 Unicode
%- motivate why asking question is important
%- grace hopper's quote: "No computer is ever going to ask a new, reasonable question. It takes trained people to do that."
%- 
%
...
\item Concerns: dataset? Evaluation? Maybe we can extract questions from ubuntu dialogue corpus (or other corpus from IMDB like sitcoms (friends, big bang theory), OR twitter dialogues (recent msr paper on dialogue uses such datasets) % https://arxiv.org/pdf/1603.06155.pdf
\end{enumerate}
